...
export function UserDetails({ user, refetch }: Props) {
  ...
  const getInitials = (firstname?: string, lastname?: string) => {
    const fn = firstname ?? user?.name?.firstname ?? "";
    const ln = lastname ?? user?.name?.lastname ?? "";
    if (!fn && !ln) return (user?.username ?? "U").slice(0, 2).toUpperCase();
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        ...
        <Card className="md:col-span-2 lg:col-span-3">
          <CardHeader>
            ...
            <CardDescription>
              Additional account details and metadata
            </CardDescription>
          </CardHeader>
          <CardContent>
