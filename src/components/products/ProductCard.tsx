...
export default function ProductCard({
  ...
  if (imageSrc && !/^https?:\/\//i.test(imageSrc)) {
    // normalize backslashes and leading slash
    imageSrc = imageSrc.replace(/\\\\/g, "/");
    if (imageSrc.startsWith("/")) imageSrc = imageSrc.slice(1);
    ...
    <Card className="w-full h-full flex flex-col relative overflow-hidden transition-all ease-in-out duration-150 hover:shadow-lg">
      ...
      <CardHeader className="flex items-center justify-center max-h-64 min-h-64 overflow-hidden">
        {imageSrc ? (
          ...
          <div className="h-48 w-full flex items-center justify-center bg-muted text-muted-foreground">
            No image
          </div>
        )}
      </CardHeader>
