...
type CreateNewProductParams = DefaultFunctionParams<Product> & {
   title?: string;
   price?: number;
   description?: string;
   category?: string;
...
class ProductService {
    ...
    // Handler for creating new product 
      async createNewProduct({ image, price, title, category, description, formData, onSuccess, onFailure }: CreateNewProductParams): Promise<CreateNewProductResponse>{
        try {

             let response;
             if (formData) {
                // Let the browser/axios set the correct Content-Type with boundary
                response = await fsClient.post<Product>(`/products`, formData);
             } else {
                response = await fsClient.post<Product>(`/products`, {
                   image,
                   price,
                   title,
                   category,
